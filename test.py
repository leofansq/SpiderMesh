...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            conf_rgb = confusion_matrix(y_true=label, y_pred=prediction_rgb, labels=[0,1,2,3,4,5,6,7,8]) # conf is an n_class*n_class matrix, vertical axis: groundtruth, horizontal axis: prediction
            conf_total_rgb += conf_rgb
            conf_thermal = confusion_matrix(y_true=label, y_pred=prediction_thermal, labels=[0,1,2,3,4,5,6,7,8]) # conf is an n_class*n_class matrix, vertical axis: groundtruth, horizontal axis: prediction
    ...
    print("* iou per class: \n  unlabeled: %.6f, car: %.6f, person: %.6f, bike: %.6f, curve: %.6f, car_stop: %.6f, guardrail: %.6f, color_cone: %.6f, bump: %.6f" \
          %(iou_per_class_rgb[0], iou_per_class_rgb[1], iou_per_class_rgb[2], iou_per_class_rgb[3], iou_per_class_rgb[4], iou_per_class_rgb[5], iou_per_class_rgb[6], iou_per_class_rgb[7], iou_per_class_rgb[8])) 
    print("* miou: %.6f\n" %(np.mean(np.nan_to_num(iou_per_class_rgb))))

    print("################################ Thermal ##################################")
    print("* iou per class: \n  unlabeled: %.6f, car: %.6f, person: %.6f, bike: %.6f, curve: %.6f, car_stop: %.6f, guardrail: %.6f, color_cone: %.6f, bump: %.6f" \
          %(iou_per_class_thermal[0], iou_per_class_thermal[1], iou_per_class_thermal[2], iou_per_class_thermal[3], iou_per_class_thermal[4], iou_per_class_thermal[5], iou_per_class_thermal[6], iou_per_class_thermal[7], iou_per_class_thermal[8])) 
